        ...
        options={{
          ...
          headerRight: () => (
            <Pressable onPress={handleShare} hitSlop={12}>
              <Ionicons name="share-outline" size={22} color={isDark ? '#F3F4F6' : '#111827'} />
            </Pressable>
          ),
        }}
      />
        ...
        <View className="px-5 py-5">
          ...
          <View className="flex-row flex-wrap gap-1.5 mb-3">
            {post.tags.map((tag) => (
              <View
                key={tag}
                className="rounded-full bg-indigo-50 dark:bg-indigo-950 px-3 py-1"
              >
          ...
          {/* 标题 */}
          <Text className="text-xl font-bold text-gray-900 dark:text-gray-100 leading-7">
            {post.title}
          </Text>
          ...
          {/* 正文内容 */}
          <Text className="mt-4 text-base text-gray-700 dark:text-gray-300 leading-7">
            {post.summary}
          </Text>
          ...
          {/* 互动数据栏 */}
          <View className="flex-row items-center mt-6 pt-4 border-t border-gray-100 dark:border-gray-700">
            <Pressable className="flex-row items-center mr-6 active:opacity-60">
              <Ionicons name="heart-outline" size={22} color={isDark ? '#9CA3AF' : '#6B7280'} />
              <Text className="ml-1.5 text-sm text-gray-500 dark:text-gray-400">{post.likes}</Text>
            </Pressable>
            <Pressable className="flex-row items-center mr-6 active:opacity-60">
              <Ionicons name="chatbubble-outline" size={21} color={isDark ? '#9CA3AF' : '#6B7280'} />
            ...
            <Pressable className="flex-row items-center active:opacity-60">
              <Ionicons name="bookmark-outline" size={22} color={isDark ? '#9CA3AF' : '#6B7280'} />
              <Text className="ml-1.5 text-sm text-gray-500 dark:text-gray-400">收藏</Text>
            </Pressable>
            ...
            <Pressable
              ...
              onPress={handleShare}
            >
              <Ionicons name="share-outline" size={22} color={isDark ? '#9CA3AF' : '#6B7280'} />
              <Text className="ml-1.5 text-sm text-gray-500 dark:text-gray-400">分享</Text>
            </Pressable>
          </View>

          {/* Route Params 调试信息 */}
          <View className="mt-6 rounded-xl bg-gray-100 dark:bg-gray-800 p-4">
